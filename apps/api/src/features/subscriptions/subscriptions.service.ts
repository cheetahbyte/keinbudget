...
import type { CreateSubscriptionInput, RemoveSubscriptionInput } from "./subcriptions.types";
import { ABO_REPO } from "./subscriptions.tokens";
import type { SubscriptionRepo } from "./subscriptions.repo";

function toMonthlyCost(price: number, billingInterval: "monthly" | "weekly" | "yearly") {
...
export class SubscriptionService {
  constructor(@inject(ABO_REPO) private readonly repo: SubscriptionRepo) {}
  ...
  async calculateStats(userId: string) {
    ...
    const monthlyCost = subscriptions.reduce(
      (sum, subscription) =>
        sum + toMonthlyCost(subscription.price, subscription.billingInterval ?? "monthly"),
      0
    );

    return {
      averagePerSub: subscriptions.length === 0 ? 0 : monthlyCost / subscriptions.length,
